import 'package:flutter/material.dart';
import 'package:google_maps_flutter/google_maps_flutter.dart';
import 'package:provider/provider.dart';
import 'package:wanderguard_companion_app/state/homescreen_state.dart';
import 'package:wanderguard_companion_app/state/backup_companion_homescreen_state.dart';
import 'package:firebase_auth/firebase_auth.dart';
import 'package:cloud_firestore/cloud_firestore.dart';

import '../routing/account_type_checker.dart';
import '../utils/colors.dart';
import 'dialogs/waiting_dialog.dart';

class GoogleMapWidget extends StatefulWidget {
  @override
  _GoogleMapWidgetState createState() => _GoogleMapWidgetState();
}

class _GoogleMapWidgetState extends State<GoogleMapWidget> {
  late GoogleMapController _controller;
  String? _accountType;

  @override
  void initState() {
    super.initState();
    _getAccountType();
  }

  Future<void> _getAccountType() async {
    String? accountType = await getCurrentUserAccountType();
    setState(() {
      _accountType = accountType;
    });
  }

  @override
  Widget build(BuildContext context) {
    if (_accountType == null) {
      return WaitingDialog(
        prompt: "Loading...",
        color: CustomColors.primaryColor,
      );
    }

    if (_accountType == 'primary_companion') {
      final homeScreenState = Provider.of<HomeScreenState>(context);
      return GoogleMap(
        initialCameraPosition: homeScreenState.initialPosition,
        mapType: MapType.normal,
        myLocationEnabled: true,
        myLocationButtonEnabled: true,
        zoomControlsEnabled: false,
        markers: homeScreenState.markers,
        circles: homeScreenState.circles,
        onMapCreated: (GoogleMapController controller) {
          _controller = controller;
          homeScreenState.setMapController(controller);
        },
      );
    } else if (_accountType == 'backup_companion') {
      final backupCompanionHomeScreenState =
          Provider.of<BackupCompanionHomeScreenState>(context);
      return GoogleMap(
        initialCameraPosition: backupCompanionHomeScreenState.initialPosition,
        mapType: MapType.normal,
        myLocationEnabled: true,
        myLocationButtonEnabled: true,
        zoomControlsEnabled: false,
        markers: backupCompanionHomeScreenState.markers,
        circles: backupCompanionHomeScreenState.circles,
        onMapCreated: (GoogleMapController controller) {
          _controller = controller;
          backupCompanionHomeScreenState.setMapController(controller);
        },
      );
    } else {
      return Center(child: Text('Unknown account type'));
    }
  }
}
